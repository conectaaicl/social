...
interface Tenant {
  ...
  domain: string | null; notes: string | null; whatsappPhone: string | null; whatsappInstance: string | null
...
function NewTenantModal({ onClose, onCreated }: { onClose: () => void; onCreated: (msg: string) => void }) {
  const [form, setForm] = useState({ tenantName: "", domain: "", email: "", name: "", password: "", plan: "PRO" })
      ...
      <div className="bg-gray-900 border border-gray-700 rounded-2xl p-6 w-full max-w-md shadow-2xl">
        ...
        <form onSubmit={submit} className="space-y-4">
          {[
            ...
            { k: "domain", label: "Dominio", placeholder: "miempresa.cl (opcional)" },
    ...
    <div className={`rounded-2xl border ${tenant.active ? "border-gray-700/50" : "border-red-900/30 opacity-70"} bg-gray-800/40 backdrop-blur p-5 space-y-4`}>
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-gray-500 mt-0.5">slug: {tenant.slug}{tenant.domain ? " · " + tenant.domain : ""} · creado {fmtDate(tenant.createdAt)}</p>
            ...
            {tenant.users.map(u => (
              <div key={u.id} className="flex items-center justify-between bg-gray-900/50 rounded-lg px-3 py-2">
                ...
                <div className="flex items-center gap-1.5 shrink-0 ml-2">
                  ...
                  <button onClick={() => { setSendCredsUser(u); setSendCredsPwd("") }} title="Enviar credenciales"
                    className="p-1 rounded hover:bg-gray-700 text-gray-600 hover:text-indigo-400 transition-colors">
      ...
      {sendCredsUser && (
        <div className="border border-indigo-500/20 bg-indigo-500/5 rounded-xl p-4 space-y-3">
          <p className="text-xs font-semibold text-indigo-400 flex items-center gap-1"><Mail className="w-3 h-3" /> Enviar credenciales — {sendCredsUser.email}</p>
          <p className="text-xs text-gray-500">Ingresa la contraseña para incluirla en el email. Si no la sabes, primero resetéala arriba.</p>
          <div className="flex gap-2">
            <input type="text" value={sendCredsPwd ?? ""} onChange={e => setSendCredsPwd(e.target.value)} placeholder="Contraseña actual del usuario"
              className="flex-1 bg-gray-900 border border-gray-700 rounded-lg px-3 py-2 text-xs text-gray-200 focus:outline-none focus:border-indigo-500" />
            <button onClick={doSendCreds} disabled={sendingCreds || !sendCredsPwd}
              className="bg-indigo-500/20 hover:bg-indigo-500/30 text-indigo-300 border border-indigo-500/30 rounded-lg px-3 py-2 text-xs font-medium disabled:opacity-40 flex items-center gap-1">
              {sendingCreds ? <RefreshCw className="w-3.5 h-3.5 animate-spin" /> : <Send className="w-3.5 h-3.5" />}
              {sendingCreds ? "Enviando..." : "Enviar"}
            </button>
            <button onClick={() => { setSendCredsUser(null); setSendCredsPwd("") }} className="text-gray-500 hover:text-gray-300 text-lg leading-none px-2">×</button>
          </div>
        </div>
      )}
    </div>
