...
export async function POST(req: NextRequest) {
  ...
  let rawUrls: string[] = []
  try {
    rawUrls = await Promise.all(imagePromises)
  } catch {
    // Fallback: generate one image and use it for all slides
    const single = await generateImage(content.imagePrompt, "CAROUSEL")
    rawUrls = Array(slideCount).fill(single)
  }
  // Persist all fal.ai URLs locally so they never expire before publishing
  const mediaUrls = await Promise.all(rawUrls.map(persistExternalImage))
